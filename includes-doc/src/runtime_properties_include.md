...
|-----------------------------------------------------|--------------------------------------------|-------------|
| TRIBEFIRE_SERVICES_URL*                             | `http://localhost:8080/tribefire-services`   | Explicit configuration of the absolute URL LoF of <b>tribefire-services</b> is required.                                         |
...
| Property                               | Local Setup Default        | Description 
|----------------------------------------|----------------------------|-------------------------------------------------------------------------|
| TRIBEFIRE_PUBLIC_SERVICES_URL*         | `/tribefire-services`      | The external available RPC endpoint of `tribefire-services`. | 
...
|TRIBEFIRE_CONTAINER_ROOT_DIR|TRIBEFIRE_INSTALLATION_ROOT_DIR`/runtime/host`|The base path where the `host/` folder of Tomcat or other servlet container is located.|
...
|Property|Local Setup Default|Description|
|---------|------------------------|---------------------------------------------------------------------|
|TRIBEFIRE_HOSTNAME | Set by TF at runtime. | The hostname assigned to the local address.|
|TRIBEFIRE_IP4_ADDRESS| Set by TF at runtime. | The IPv4 address of the host. TF will try to ignore loopback adapters and network adapters that are deactivated. Furthermore, it will check whether the address is reachable. There is also the possibility to provide a blacklist of network interfaces. If there are multiple addresses, TF will take the first one available.|
|TRIBEFIRE_IP6_ADDRESS| Set by TF at runtime. | The IPv6 of the host, obeying the same rules as for IPv4 addresses.|
|TRIBEFIRE_IP_ADDRESS| Set by TF at runtime. | The same as the IPv4 address, unless `TRIBEFIRE_PREFER_IPV6` is true.|
|TRIBEFIRE_JVM_UUID| Set by TF at runtime. | Unique ID used to construct the Node ID together with the hostname. |
...
|TRIBEFIRE_NODE_ID | Set by TF at runtime. | An ID constructed from the following components (explained above): `${TRIBEFIRE_PROJECT_NAME}@${TRIBEFIRE_HOSTNAME}#${TRIBEFIRE_JVM_UUID}`|
